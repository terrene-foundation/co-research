...
### 2. Inherited Template Placeholders

This repository was instantiated from a configurable template. If an inherited artifact still carries placeholder markers, they are unfilled configuration — fill them with this domain's specifics:

```markdown
# DO:

Replace [Your Domain], [Domain], [key quality], [key risk] with this domain's specifics

# DO NOT:

Leave [Your Domain] or [Domain] markers live in a canonical artifact
```

**Why**: A surviving placeholder is unfilled template configuration — once the artifact is canonical here, it is a `no-stubs.md` MUST §1 violation that ships inconsistent behavior.
