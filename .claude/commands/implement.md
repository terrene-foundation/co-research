...
Execute the work one task at a time from the approved roadmap. Each run of `/implement` completes one task — drafting documents, analyzing structures, creating specifications, or building content.

## Your Role (communicate to user)

Your role is to answer questions when decisions come up — these will always be about what the content should say, what position to take, or what stakeholders need. You can check progress anytime with `/ws`.
