---
name: sweep
description: "Repo-wide outstanding-work audit — workspaces, issues, validation gaps. End-of-cycle gate before /wrapup."
---

## Purpose

A `/sweep` is the structural defense against "I think we're done." Before declaring a session converged or starting fresh work, surface every class of outstanding item: in-flight todos, open issues, validation gaps against specs, stale workspace state, and process-hygiene gaps.

Distinct from the phase-04 review commands (`/redteam`, `/preflight` — each scopes to ONE workspace's validation) — `/sweep` is repo-wide and rolls every workspace's validation status into one view.
...
### Sweep 2: Pending journal entries (awaiting promotion)

```bash
find workspaces/[^_]*/journal/.pending/ -name "*.md" 2>/dev/null
```

Per `rules/journal.md` entry discipline: high-value session insight → promote to a numbered entry (sequential `NNNN-` naming), redundant → discard with note, already covered by another entry → discard with cross-reference.

### Sweep 3: GitHub open issues — this repository's tracker
...
### Sweep 5: Validation gaps against specs (every workspace)

Phase-04 validation re-derived as a repo-wide sweep. Use `rules/specs-authority.md` protocol — verify field-level assertions, never file existence.
...
Per workspace, per spec file: verify every MUST clause and assertion has a corresponding deliverable in `03-drafts/` or a validation record in `04-validate/`. Categorize findings:
...
- **Stale spec** — spec written, never validated (>14d since spec creation, no `04-validate/` output for it)
- **Cross-spec inconsistency** — two specs in the same workspace contradict on shared terms
- **Placeholder leak** — `[TODO]`, `[TBD]`, `[FIXME]` left in `03-drafts/` (`rules/no-stubs.md` MUST §1)
...
find workspaces/[^_]*/journal/.pending/*.md -mtime +14 2>/dev/null  # stale .pending
find workspaces/_template/01-analyze -newer workspaces/_template/briefs 2>/dev/null  # template drift
```

Surface: workspaces with `.session-notes` >30d (archive or close), worktrees not at HEAD or zero-commit (cleanup), `.pending` >14d (promote OR discard), template directories newer than the template's briefs dir (template drifted from documented baseline).
...
Write findings to `workspaces/<project>/04-validate/sweep-<date>.md` (workspace context active) OR `SWEEP-<date>.md` at the repo root. Each finding:
...
Origin: adopted from atelier canonical sweep.md (GH atelier#15) — hub-only sweeps (sibling-repo scans, downstream sync currency, distribution targets) dropped; workspace dirs remapped to this repo's template contract (01-analyze, 03-drafts, 04-validate, briefs).
